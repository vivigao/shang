...
bool Model::ParseLine(const std::vector<ci_string> &token){
	if (token.empty() == false){
		if (token[0].empty() == false && token[0][0] == '#')
			...
			//vec3 p = mAnimation->GetCurrentPosition(dt);

			//Collision *c = new Collision();
			//bool isC = emitParticle(p);
			if(dt==0){
...
bool Model::emitParticle(glm::vec3 p){
...
vec3 Model::GetCurrentPosition(int dt)
{ vec3 currentPositon;
 currentPositon = mAnimation->GetCurrentPosition(dt);
